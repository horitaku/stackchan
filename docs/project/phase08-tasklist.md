...
| P8-19 | runtime_metrics に watermark 状態統合する | firmware からの watermark イベント送信、server 側 metrics 記録処理、WebUI Overview パネル拡張、可視化ダッシュボード | 低 | ネットワーク揺らぎの履歴分析と定量的な診断を可能にし、運用時の最適化判定を支援するため | Done |
...
## 2.7 P8-19 runtime_metrics に watermark 状態統合メモ（2026-03-19）

### 実装内容

#### Protocol 層
- `protocol/websocket/schemas/tts.buffer.watermark.schema.json` を作成
  - envelope 参照 (allOf)
  - payload: request_id, stream_id, status (enum: normal|low_water|high_water), buffered_ms, threshold_ms, frames_in_queue
- `protocol/examples/tts.buffer.watermark.example.json` を作成
  - status: "low_water", buffered_ms: 40, threshold_ms: 60, frames_in_queue: 2
- `protocol/websocket/events.md` Section 5.11 に事象定義を追加
  - 方向: firmware → server
  - 目的: ネットワーク揺らぎの履歴分析と定量診断

#### Server 層
- `server/internal/protocol/events.go` に `TTSBufferWatermarkPayload` struct 追加
  - RequestID, StreamID, Status, BufferedMs, ThresholdMs, FramesInQueue
- `server/internal/web/runtime_state.go` で `PipelineSnapshot` を拡張
  - `TTSWatermarkStatus` (string)
  - `TTSBufferedMs` (int)
  - `TTSLowWaterCount` (int)
  - `TTSHighWaterDropCount` (int)
  - `OnTTSWatermark()` メソッドを実装: 状態解析 → カウンタ更新 → 4 メトリクスを `runtime_metrics` テーブルへ永続化
- `server/internal/web/ws_handler.go` に case "tts.buffer.watermark" ハンドラを追加
  - payload unmarshal → 必須フィールド検証 → `OnTTSWatermark()` へ委譲

#### Firmware 層
- `firmware/protocol/events.h` に `TTS_BUFFER_WATERMARK = "tts.buffer.watermark"` 定数を追加
- `firmware/app/stackchan/session.h` に watermark 追跡を追加
  - `_ttsWatermarkStatus` (現在の状態)
  - `_ttsWatermarkLastSentMs` (最後の送信時刻)
  - `kWatermarkCooldownMs = 500` (再送制限)
  - `sendTTSBufferWatermark()` メソッド実装
- `firmware/app/stackchan/session.cpp` で:
  - `sendTTSBufferWatermark()` が JSON ペイロード構築 → envelope 化 → WebSocket 送信
  - cooldown チェック: 同一状態 & 500ms 以内なら送信スキップ
  - `processTTSPlaybackQueue()` で low-water 検知時に送信
  - `enqueueTTSFrame()` で high-water drop 時に送信
  - normal 状態への回復を検知して "normal" イベント送信

#### WebUI 層
- `server/webui/src/App.svelte` の "会話パイプライン" カードに 4 フィールド表示を追加
  - `TTS Buffer Status`: status の表示
  - `TTS Buffered`: buffered_ms (ms 単位)
  - `Low Water Events`: low_water_count (カウンタ)
  - `High Water Drops`: high_water_drop_count (カウンタ)

### ビルド & テスト結果
- firmware build: **SUCCESS**
  - RAM: 15.3% (P8-18 と変わらず)
  - Flash: 17.8% (P8-18 と変わらず)
  - Build time: 19.49 秒
- server tests: **PASS**
  - 全テスト成功（protocol unmarshal, OnTTSWatermark 処理、handler 統合含む）

### Watermark Design Summary
| 項目 | 値 | 備考 |
|-----|-----|------|
| Low Water Threshold | 60ms | バッファ枯渇警告レベル |
| High Water Threshold | 240ms | バッファ過剰・フレームドロップレベル |
| Cooldown | 500ms | 同一状態での再送間隔制限 |
| Metrics Persistence | runtime_metrics DB | 4 メトリクスを永続化 |
| API Exposure | GET /api/runtime/overview | snapshot.pipeline に統合 |
| WebUI Display | Overview card | 4 フィールドをリアルタイム表示 |

### 今後の検証ステップ
1. 実機 (M5Stack CoreS3) で Stackchan firmware を実行
2. server を docker-compose で起動
3. 通常会話で `tts.buffer.watermark` イベントが送受信されるか確認
4. runtime_metrics テーブルに watermark レコードが記録されるか確認
5. WebUI Overview パネルでカウンタが増分されるか確認
6. Wi-Fi に人為的に遅延を導入して watermark 状態遷移を検出

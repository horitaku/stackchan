...
struct ServoAxisCalibration {
  ...
  bool  soft_start{true};            ///< ソフトスタート（スムーズな加減速）有効フラグ
...
class ServoController {
 ...
 private:
  ...
  /**
   * @brief 1 軸の校正値を Preferences に書き込みます。
   *
   * @param ns    Preferences の名前空間（"servo_x" または "servo_y"）
   * @param cal   保存する校正値
   * @return 保存成功: true、失敗: false
   */
  bool saveCalibration(const char* ns, const ServoAxisCalibration& cal);
